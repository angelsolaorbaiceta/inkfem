...
func TestLoadIsNodal(t *testing.T) {
    if load := MakeConcentrated(FX, true, inkgeom.MakeTParam(inkgeom.MIN_T), 45.0); !load.IsNodal() {
        t.Error("Expected load to be nodal (t = 0.0)")
    }

    if load := MakeConcentrated(FX, true, inkgeom.MakeTParam(inkgeom.MAX_T), 45.0); !load.IsNodal() {
