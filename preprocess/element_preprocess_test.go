...
func TestSliceAxialMemberNodePositions(t *testing.T) {
	...
	if pos := slicedEl.NodeAt(0).Position; !pos.Equals(element.StartPoint()) {
		t.Errorf("Wrong node position. Want %v, got %v", element.StartPoint(), pos)
	}
	if pos := slicedEl.NodeAt(1).Position; !pos.Equals(element.EndPoint()) {
...
func TestSliceAxialMemberNodeLoads(t *testing.T) {
	...
	t.Run("Start node loads", func(t *testing.T) {
		if slicedEl.NodeAt(0).NetLocalFx() != 50.0 {
			t.Error("Node Fx value not as expected")
		}
		if slicedEl.NodeAt(0).NetLocalFy() != 75.0 {
			t.Error("Node Fy value not as expected")
		}
	})

	t.Run("End node loads", func(t *testing.T) {
		if slicedEl.NodeAt(1).NetLocalFx() != 100.0 {
			t.Error("Node Fx value not as expected")
		}
		if slicedEl.NodeAt(1).NetLocalFy() != 200.0 {
			t.Error("Node Fy value not as expected")
		}
	})
}
...
func TestSliceAxialMemberGlobalLoadProjected(t *testing.T) {
	...
	if fx := slicedEl.NodeAt(0).NetLocalFx(); !nums.FloatsEqual(fx, expectedProjLoadX) {
		t.Error("Node projected Fx value was not as expected")
	}
	if fy := slicedEl.NodeAt(0).NetLocalFy(); !nums.FloatsEqual(fy, expectedProjLoadY) {
...
func TestSliceNonAxialUnloadedMemberNodePositions(t *testing.T) {
	...
	if pos := slicedEl.NodeAt(0).Position; !pos.Equals(wantStartPoint) {
		t.Error("First node's position was not as expected")
	}
	if pos := slicedEl.NodeAt(1).Position; !pos.Equals(wantMidPoint) {
		t.Error("Middle node's position was not as expected")
	}
	if pos := slicedEl.NodeAt(2).Position; !pos.Equals(wantEndPoint) {
...
func TestDistributedGlobalLoadDistribution(t *testing.T) {
	...
	// First Node
	if fx := slicedEl.NodeAt(0).NetLocalFx(); !nums.FloatsEqual(fx, 5.0) {
		t.Errorf("First node Fx expected to be 5.0, but was %f", fx)
	}
	if fy := slicedEl.NodeAt(0).NetLocalFy(); !nums.FloatsEqual(fy, 5.0) {
		t.Errorf("First node Fy expected to be 5.0, but was %f", fy)
	}
	if mz, expected := slicedEl.NodeAt(0).NetLocalMz(), 10.0/math.Sqrt(18.0); !nums.FloatsEqual(mz, expected) {
		t.Errorf("First node Mz expected to be %f, but was %f", expected, mz)
	}

	// Second Node
	if fx := slicedEl.NodeAt(1).NetLocalFx(); !nums.FloatsEqual(fx, 10.0) {
		t.Errorf("Second node Fx expected to be 10.0, but was %f", fx)
	}
	if fy := slicedEl.NodeAt(1).NetLocalFy(); !nums.FloatsEqual(fy, 10.0) {
		t.Errorf("Second node Fy expected to be 10.0, but was %f", fy)
	}
	if mz := slicedEl.NodeAt(1).NetLocalMz(); mz != 0.0 {
		t.Errorf("Second node Mz expected to be 0.0, but was %f", mz)
	...
	// Third Node
	if fx := slicedEl.NodeAt(2).NetLocalFx(); !nums.FloatsEqual(fx, 5.0) {
		t.Errorf("Third node Fx expected to be 5.0, but was %f", fx)
	}
	if fy := slicedEl.NodeAt(2).NetLocalFy(); !nums.FloatsEqual(fy, 5.0) {
		t.Errorf("Third node Fy expected to be 5.0, but was %f", fy)
	}
	if mz, expected := slicedEl.NodeAt(2).NetLocalMz(), -10.0/math.Sqrt(18.0); !nums.FloatsEqual(mz, expected) {
...
func TestConcentratedLocalLoadDistribution(t *testing.T) {
	...
	if fx := slicedEl.NodeAt(1).NetLocalFx(); fx != 3.0 {
		t.Errorf("First node Fx expected to be 3.0, but was %f", fx)
	}
	if fy := slicedEl.NodeAt(1).NetLocalFy(); fy != 5.0 {
		t.Errorf("First node Fy expected to be 5.0, but was %f", fy)
	}
	if mz := slicedEl.NodeAt(1).NetLocalMz(); mz != 7.0 {
...
func TestConcentratedGlobalLoadDistribution(t *testing.T) {
	...
	if fx := slicedEl.NodeAt(1).NetLocalFx(); !nums.FloatsEqual(fx, 5.0/math.Sqrt2) {
		t.Errorf("First node Fx expected to be %f, but was %f", 5.0/math.Sqrt2, fx)
	}
	if fy := slicedEl.NodeAt(1).NetLocalFy(); !nums.FloatsEqual(fy, 5.0/math.Sqrt2) {
